...
def classify_intent(text: str) -> tuple[str,str]:
    active_model = os.getenv("MEDCHAT_MODEL", "rule")
    if active_model == "rule":
        lowered = text.lower() 
        if any(x in lowered for x in ["kill myself","end it","suicide"]):
            return "crisis","rule-based"
    
        if any(x in lowered for x in ["sad","tired","stressed","anxious"]):
            return "emotional","rule-based"
        
        if len(text.split()) > 15:
            return "venting", "rule-based"
        
        return "neutral", "rule-based"
    
    if active_model == "dummy":
        time.sleep(random.uniform(0.5,0.2)) 
        return random.choice(
            ["neutral", "venting", "emotional"]
        ), "dummy-ml"
